...
import com.unisys.aos.view.messageprocessor.entity.PSHT;
...
@Service("FLOP_PSHT")
@Slf4j
public class PshtProcessor extends AbsProcessor implements Processor {
    private final FlightDAO flightDAO;

    @Autowired
    public PshtProcessor(FlightDAO flightDAO) {
        this.flightDAO = flightDAO;
    }

    /**
     * Process the PSHT message.
     *
    ...
    public void process(String msg) throws JsonProcessingException {
        ...
        if (null == flops || flops.isEmpty()) {
            log.error("There is no FLOP section in PSHT message. Message text [" + msg + "]");
            return;
        }
        ...
        for (FLOP.FlopData record : flops) {
            if (null == record.getFLID()) {
                log.error("There is no FFID tag in PSHT message. Message text [" + msg + "]");
                return;
            }
            ...
            try {
                List<FmsStand> stands = parseStandUpdate(record.getPshts(), record.getFLID());
            ...
            for (FmsStand stand : flightDTO.getStands()){
                ...
                if (null == standUpdate.getPushTime()){
                    if (null == stand.getPushTime()){
                        continue;
                    }
                    standUpdate.setPushIndicator(null);
                }else if (standUpdate.getPushTime().equals(stand.getPushTime()) &&
                        (null == standUpdate.getPushIndicator()? null == stand.getPushIndicator() :
                                standUpdate.getPushIndicator().equals(stand.getPushIndicator()))) {
                    continue;
                }
                stand.setPushTime(standUpdate.getPushTime());
                stand.setPushIndicator(standUpdate.getPushIndicator());
            ...
            if(stands.size()!=0) {
                ...
                FlightNotificationDTO dto = FlightNotificationDTO.buildUpdateNotification(record.getFLID())
                        .addUpdateFields("standPushTime", stands);
                publishToRedis(dto);
            }
        }
    }
