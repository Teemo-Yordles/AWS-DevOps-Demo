...
/**
 * flight currentStand type change message processor
 *
...
@Service("FLOP_STND")
@Slf4j
public class StndProcessor extends AbsProcessor implements Processor {
    private final FlightDAO flightDAO;

    @Autowired
    public StndProcessor(FlightDAO flightDAO) {
        this.flightDAO = flightDAO;
    }

    /**
     * Process the STND message.
     *
    ...
    public void process(String msg) throws JsonProcessingException {
        ...
        if (null == flops || flops.isEmpty()) {
            log.error("There is no FLOP section in STND message. Message text [" + msg + "]");
            return;
        }
        ...
        for (FLOP.FlopData record : flops) {
            if (null == record.getFLID()) {
                log.error("There is no FFID tag in STND message. Message text [" + msg + "]");
                return;
            }
            ...
            // check maxp tag
            String currentStandCode = record.getSTND();
            if (null == currentStandCode) {
                log.error("STND cannot be empty in STND message. Message text [" + msg + "]");
                return;
            }
            // if same, no update is made.
            if (currentStandCode.equals(flightDTO.getFlight().getCurrentStand())) {
                log.info("STND is same so nothing changed. CurrentStandCode=[{}]", currentStandCode);
                return;
            }
            // do actual update
            FlightDTO newFlightDTO = (FlightDTO) ObjectUtil.deepClone(flightDTO);
            if (null != newFlightDTO && null != newFlightDTO.getFlight()) {
                newFlightDTO.getFlight().setCurrentStand(currentStandCode);
                ...
                FlightNotificationDTO dto = FlightNotificationDTO.buildUpdateNotification(record.getFLID())
                        .addUpdateFields("currentStandCode",currentStandCode);
                publishToRedis(dto);
            } else {
