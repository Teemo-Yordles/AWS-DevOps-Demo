...
/**
 * flight registration type change message processor
 *
...
@Service("FLOP_RENO")
@Slf4j
public class RenoProcessor extends AbsProcessor implements Processor {
    private final FlightDAO flightDAO;

    @Autowired
    public RenoProcessor(FlightDAO flightDAO) {
        this.flightDAO = flightDAO;
    }

    /**
     * Process the RENO message.
     *
    ...
    public void process(String msg) throws JsonProcessingException {
        ...
        if (null == flops || flops.isEmpty()) {
            log.error("There is no FLOP section in RENO message. Message text [" + msg + "]");
            return;
        }
        ...
        for (FLOP.FlopData record : flops) {
            if (null == record.getFLID()) {
                log.error("There is no FFID tag in RENO message. Message text [" + msg + "]");
                return;
            }
            ...
            // check RENO tag
            String registrationCode = record.getRENO();
            if (null == registrationCode) {
                log.error("RENO cannot be empty in RENO message. Message text [" + msg + "]");
                return;
            }
            // if same, no update is made.
            if (registrationCode.equals(flightDTO.getFlight().getRegistration())) {
                log.info("RENO is same so nothing changed. RegistrationCode=[{}]", registrationCode);
                return;
            }
            // do actual update
            FlightDTO newFlightDTO = (FlightDTO) ObjectUtil.deepClone(flightDTO);
            if (null != newFlightDTO && null != newFlightDTO.getFlight()) {
                newFlightDTO.getFlight().setRegistration(registrationCode);
                ...
                FlightNotificationDTO dto = FlightNotificationDTO.buildUpdateNotification(record.getFLID())
                        .addUpdateFields("registrationCode",registrationCode);
                publishToRedis(dto);
            } else {
