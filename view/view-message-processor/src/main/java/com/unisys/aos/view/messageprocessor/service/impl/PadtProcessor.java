...
/**
 * flight padt type change message processor
 *
...
@Service("FLOP_PADT")
@Slf4j
public class PadtProcessor extends AbsProcessor implements Processor {
    private final FlightDAO flightDAO;

    @Autowired
    public PadtProcessor(FlightDAO flightDAO) {
        this.flightDAO = flightDAO;
    }

    /**
     * Process the PADT message.
     *
    ...
    public void process(String msg) throws JsonProcessingException {
        ...
        if (null == flops || flops.isEmpty()) {
            log.error("There is no FLOP section in PADT message. Message text [" + msg + "]");
            return;
        }
        ...
        for (FLOP.FlopData record : flops) {
            if (null == record.getFLID()) {
                log.error("There is no FFID tag in PADT message. Message text [" + msg + "]");
                return;
            }
            ...
            // check PADT tag
            String padt = record.getPADT();
            Date padtCode = null;
            if(padt == null){
                if (flightDTO.getFlight().getPadt() == null){
                    log.info("PADT is same so nothing changed. PadtCode=null");
                    return;
                }
            }else{
                try {
                    padtCode = ThreadLocalDateUtil.parseXmlDate(padt);
                } catch (ParseException e) {
                    log.error("Parser PADT xml date error.Message text [" + msg + "]" );
                    return;
                }
                // if same, no update is made.
                if (padtCode.equals(flightDTO.getFlight().getPadt())) {
                    log.info("PADT is same so nothing changed. PadtCode=[{}]", padtCode);
                    return;
                }
            }
            ...
            if (null != newFlightDTO && null != newFlightDTO.getFlight()) {
                newFlightDTO.getFlight().setPadt(padtCode);
                ...
                FlightNotificationDTO dto = FlightNotificationDTO.buildUpdateNotification(record.getFLID())
                        .addUpdateFields("padtCode",padtCode);
                publishToRedis(dto);
            } else {
