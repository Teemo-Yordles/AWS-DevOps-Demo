...
/**
 * flight tobt type change message processor
 *
...
@Service("FLOP_TOBT")
@Slf4j
public class TobtProcessor extends AbsProcessor implements Processor {
    private final FlightDAO flightDAO;

    @Autowired
    public TobtProcessor(FlightDAO flightDAO) {
        this.flightDAO = flightDAO;
    }

    /**
     * Process the TOBT message.
     *
    ...
    public void process(String msg) throws JsonProcessingException {
        ...
        if (null == flops || flops.isEmpty()) {
            log.error("There is no FLOP section in TOBT message. Message text [" + msg + "]");
            return;
        }
        ...
        for (FLOP.FlopData record : flops) {
            if (null == record.getFLID()) {
                log.error("There is no FFID tag in TOBT message. Message text [" + msg + "]");
                return;
            }
            ...
            // check TOBT tag
            String tobt = record.getTOBT();
            Date tobtCode = null;
            if(tobt == null){
                if (flightDTO.getFlight().getTargetOffblock() == null){
                    log.info("TOBT is same so nothing changed. TobtCode=null");
                    return;
                }
            }else{
                try {
                    tobtCode = ThreadLocalDateUtil.parseXmlDate(tobt);
                } catch (ParseException e) {
                    log.error("Parser TOBT xml date error.Message text [" + msg + "]" );
                    return;
                }
                // if same, no update is made.
                if (tobtCode.equals(flightDTO.getFlight().getTargetOffblock())) {
                    log.info("TOBT is same so nothing changed. TobtCode=[{}]", tobtCode);
                    return;
                }
            }
            ...
            if (null != newFlightDTO && null != newFlightDTO.getFlight()) {
                newFlightDTO.getFlight().setTargetOffblock(tobtCode);
                ...
                FlightNotificationDTO dto = FlightNotificationDTO.buildUpdateNotification(record.getFLID())
                        .addUpdateFields("tobtCode",tobtCode);
                publishToRedis(dto);
            } else {
