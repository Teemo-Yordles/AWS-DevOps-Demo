...
/**
 * flight flightIndicator type change message processor
 *
...
@Service("FLOP_FLIN")
@Slf4j
public class FlinProcessor extends AbsProcessor implements Processor {
    private final FlightDAO flightDAO;

    @Autowired
    public FlinProcessor(FlightDAO flightDAO) {
        this.flightDAO = flightDAO;
    }

    /**
     * Process the FLIN message.
     *
    ...
    public void process(String msg) throws JsonProcessingException {
        ...
        if (null == flops || flops.isEmpty()) {
            log.error("There is no FLOP section in FLIN message. Message text [" + msg + "]");
            return;
        }
        ...
        for (FLOP.FlopData record : flops) {
            if (null == record.getFLID()) {
                log.error("There is no FFID tag in FLIN message. Message text [" + msg + "]");
                return;
            }
            ...
            // check FLIN tag
            Character flightIndicatorCode = record.getFLIN();
            if (null == flightIndicatorCode) {
                log.error("FLIN cannot be empty in FLIN message. Message text [" + msg + "]");
                return;
            }
            // if same, no update is made.
            if (String.valueOf(flightIndicatorCode).equals(flightDTO.getFlight().getFlightIndicator())) {
                log.info("FLIN is same so nothing changed. FlightIndicatorCode=[{}]", flightIndicatorCode);
                return;
            }
            // do actual update
            FlightDTO newFlightDTO = (FlightDTO) ObjectUtil.deepClone(flightDTO);
            if (null != newFlightDTO && null != newFlightDTO.getFlight()) {
                newFlightDTO.getFlight().setFlightIndicator(String.valueOf(flightIndicatorCode));
                ...
                FlightNotificationDTO dto = FlightNotificationDTO.buildUpdateNotification(record.getFLID())
                        .addUpdateFields("flightIndicatorCode",flightIndicatorCode);
                publishToRedis(dto);
            } else {
