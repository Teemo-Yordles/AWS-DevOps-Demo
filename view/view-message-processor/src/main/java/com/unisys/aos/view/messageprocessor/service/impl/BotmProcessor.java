...
/**
 * flight boarding time change message processor
 *
...
@Service("FLOP_BOTM")
@Slf4j
public class BotmProcessor extends AbsProcessor {
    private final FlightDAO flightDAO;

    @Autowired
    public BotmProcessor(FlightDAO flightDAO) {
        this.flightDAO = flightDAO;
    }

    /**
     * Process the BOTM message.
     *
        ...
        if (null != flops && !flops.isEmpty()) {
            for (FLOP.FlopData record : flops) {
                if (null == record.getFLID()) {
                    log.error("There is no FFID tag in BOTM message. Message text [" + msg + "]");
                    return;
                }
                ...
                // check flight boarding time tag
                String boardingTimeStr = record.getBOTM();
                if (null == boardingTimeStr || boardingTimeStr.isEmpty()) {
                    log.info("The existing boarding time is being blanked out. Message text [" + msg + "]");
                    if (null == flightDTO.getFlight().getBoardingOpen()) {
                        return;
                    }
                    updateBotm(flightDTO, null);
                    return;
                }
                try {
                    Date boardingTime = ThreadLocalDateUtil.parseXmlDate(boardingTimeStr);
                    if (null == boardingTime) {
                        log.error("The format of BOTM is wrong, Message text [" + msg + "], BOTM text [" + boardingTimeStr + "]");
                        return;
                    }
                    // if same, no update is made.
                    Date oldBoardingTime = flightDTO.getFlight().getBoardingOpen();
                    if (null != oldBoardingTime && boardingTime.compareTo(oldBoardingTime) == 0) {
                        log.info("BOTM is same so nothing changed. Final time=[{}]", boardingTime);
                    } else {
                        // do actual update
                        updateBotm(flightDTO, boardingTime);
                    }
            ...
            log.error("There is no FLOP section in BOTM message. Message text [" + msg + "]");
        }
    }
     ...
     * @param boardingTime flight boarding time.
     */
    private void updateBotm(FlightDTO flightDTO, Date boardingTime) {
        FlightDTO newFlightDTO = (FlightDTO) ObjectUtil.deepClone(flightDTO);
        if (null != newFlightDTO && null != newFlightDTO.getFlight()) {
            newFlightDTO.getFlight().setBoardingOpen(boardingTime);
            ...
            FlightNotificationDTO dto = FlightNotificationDTO.buildUpdateNotification(flightDTO.getFlight().getId())
                    .addUpdateFields("boardingTime",boardingTime);
            publishToRedis(dto);
        } else {
