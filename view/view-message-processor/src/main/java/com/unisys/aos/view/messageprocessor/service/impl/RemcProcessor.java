...
/**
 * flight remarks type change message processor
 *
...
@Service("FLOP_REMC")
@Slf4j
public class RemcProcessor extends AbsProcessor implements Processor {
    private final FlightDAO flightDAO;

    @Autowired
    public RemcProcessor(FlightDAO flightDAO) {
        this.flightDAO = flightDAO;
    }

    /**
     * Process the REMC message.
     *
    ...
    public void process(String msg) throws JsonProcessingException {
        ...
        if (null == flops || flops.isEmpty()) {
            log.error("There is no FLOP section in REMC message. Message text [" + msg + "]");
            return;
        }
        ...
        for (FLOP.FlopData record : flops) {
            if (null == record.getFLID()) {
                log.error("There is no FFID tag in REMC message. Message text [" + msg + "]");
                return;
            }
            ...
            // check REMC tag
            String remarksCode = record.getREMC();
            if (null == remarksCode) {
                log.error("REMC cannot be empty in REMC message. Message text [" + msg + "]");
                return;
            }
            // if same, no update is made.
            if (remarksCode.equals(flightDTO.getFlight().getRemarks())) {
                log.info("REMC is same so nothing changed. RemarksCode=[{}]", remarksCode);
                return;
            }
            // do actual update
            FlightDTO newFlightDTO = (FlightDTO) ObjectUtil.deepClone(flightDTO);
            if (null != newFlightDTO && null != newFlightDTO.getFlight()) {
                newFlightDTO.getFlight().setRemarks(remarksCode);
                ...
                FlightNotificationDTO dto = FlightNotificationDTO.buildUpdateNotification(record.getFLID())
                        .addUpdateFields("remarksCode",remarksCode);
                publishToRedis(dto);
            } else {
