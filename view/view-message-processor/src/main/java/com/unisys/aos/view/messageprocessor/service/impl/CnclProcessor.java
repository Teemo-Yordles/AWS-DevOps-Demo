...
/**
 * flight cancel message processor
 *
...
@Service("FLOP_CNCL")
@Slf4j
public class CnclProcessor extends AbsProcessor {
    private final FlightDAO flightDAO;

    @Autowired
    public CnclProcessor(FlightDAO flightDAO) {
        this.flightDAO = flightDAO;
    }

    /**
     * Process the CNCL message.
     *
        ...
        if (null != flops && !flops.isEmpty()) {
            for (FLOP.FlopData record : flops) {
                if (null == record.getFLID()) {
                    log.error("There is no FFID tag in CNCL message. Message text [" + msg + "]");
                    return;
                }
                ...
                // check flight cancel tag
                String cancellationTimeStr = record.getCNCL();
                if (null == cancellationTimeStr || cancellationTimeStr.isEmpty()) {
                    log.info("Undo cancellation of a previously cancelled flight. Message text [" + msg + "]");
                    if (null == flightDTO.getFlight().getCancel()) {
                        return;
                    }
                    updateCncls(flightDTO, null);
                    return;
                }
                try {
                    Date cancellationTime = ThreadLocalDateUtil.parseXmlDate(cancellationTimeStr);
                    if (null == cancellationTime) {
                        log.error("The format of CNCL is wrong, Message text [" + msg + "], CNCL text [" + cancellationTimeStr + "]");
                        return;
                    }
                    // if same, no update is made.
                    Date oldCancellationTime = flightDTO.getFlight().getCancel();
                    if (null != oldCancellationTime && cancellationTime.compareTo(oldCancellationTime) == 0) {
                        log.info("CNCL is same so nothing changed. Final time=[{}]", cancellationTime);
                    } else {
                        // do actual update
                        updateCncls(flightDTO, cancellationTime);
                    }
            ...
            log.error("There is no FLOP section in CNCL message. Message text [" + msg + "]");
        }
    }
     ...
     * @param cancellationTime flight cancellation time.
     */
    private void updateCncls(FlightDTO flightDTO, Date cancellationTime) {
        FlightDTO newFlightDTO = (FlightDTO) ObjectUtil.deepClone(flightDTO);
        if (null != newFlightDTO && null != newFlightDTO.getFlight()) {
            newFlightDTO.getFlight().setCancel(cancellationTime);
            ...
            FlightNotificationDTO dto = FlightNotificationDTO.buildUpdateNotification(flightDTO.getFlight().getId())
                    .addUpdateFields("cancellationTime",cancellationTime);
            publishToRedis(dto);
        } else {
