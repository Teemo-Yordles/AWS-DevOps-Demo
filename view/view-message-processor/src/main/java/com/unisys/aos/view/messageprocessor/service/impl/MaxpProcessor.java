...
/**
 * flight maxPax type change message processor
 *
...
@Service("FLOP_MAXP")
@Slf4j
public class MaxpProcessor extends AbsProcessor implements Processor {
    private final FlightDAO flightDAO;

    @Autowired
    public MaxpProcessor(FlightDAO flightDAO) {
        this.flightDAO = flightDAO;
    }

    /**
     * Process the MAXP message.
     *
    ...
    public void process(String msg) throws JsonProcessingException {
        ...
        if (null == flops || flops.isEmpty()) {
            log.error("There is no FLOP section in MAXP message. Message text [" + msg + "]");
            return;
        }
        ...
        for (FLOP.FlopData record : flops) {
            if (null == record.getFLID()) {
                log.error("There is no FFID tag in MAXP message. Message text [" + msg + "]");
                return;
            }
            ...
            // check maxp tag
            Short maxPaxCode = record.getMAXP();
            if (null == maxPaxCode) {
                log.error("MAXP cannot be empty in MAXP message. Message text [" + msg + "]");
                return;
            }
            // if same, no update is made.
            if (maxPaxCode.equals(flightDTO.getFlight().getMaxPax())) {
                log.info("MAXP is same so nothing changed. MaxPaxCode=[{}]", maxPaxCode);
                return;
            }
            // do actual update
            FlightDTO newFlightDTO = (FlightDTO) ObjectUtil.deepClone(flightDTO);
            if (null != newFlightDTO && null != newFlightDTO.getFlight()) {
                newFlightDTO.getFlight().setMaxPax(maxPaxCode);
                ...
                FlightNotificationDTO dto = FlightNotificationDTO.buildUpdateNotification(record.getFLID())
                        .addUpdateFields("maxPaxCode",maxPaxCode);
                publishToRedis(dto);
            } else {
