...
/**
 * flight neat type change message processor
 *
...
@Service("FLOP_NEAT")
@Slf4j
public class NeatProcessor extends AbsProcessor implements Processor {
    private final FlightDAO flightDAO;

    @Autowired
    public NeatProcessor(FlightDAO flightDAO) {
        this.flightDAO = flightDAO;
    }

    /**
     * Process the NEAT message.
     *
    ...
    public void process(String msg) throws JsonProcessingException {
        ...
        if (null == flops || flops.isEmpty()) {
            log.error("There is no FLOP section in NEAT message. Message text [" + msg + "]");
            return;
        }
        ...
        for (FLOP.FlopData record : flops) {
            if (null == record.getFLID()) {
                log.error("There is no FFID tag in NEAT message. Message text [" + msg + "]");
                return;
            }
            ...
            // check NEAT tag
            String neat = record.getNEAT();
            Date neatCode = null;
            if(neat == null){
                if (flightDTO.getFlight().getNeat() == null){
                    log.info("NEAT is same so nothing changed. NeatCode=null");
                    return;
                }
            }else{
                try {
                    neatCode = ThreadLocalDateUtil.parseXmlDate(neat);
                } catch (ParseException e) {
                    log.error("Parser NEAT xml date error.Message text [" + msg + "]" );
                    return;
                }
                // if same, no update is made.
                if (neatCode.equals(flightDTO.getFlight().getNeat())) {
                    log.info("NEAT is same so nothing changed. NeatCode=[{}]", neatCode);
                    return;
                }
            }
            ...
            if (null != newFlightDTO && null != newFlightDTO.getFlight()) {
                newFlightDTO.getFlight().setNeat(neatCode);
                ...
                FlightNotificationDTO dto = FlightNotificationDTO.buildUpdateNotification(record.getFLID())
                        .addUpdateFields("neatCode",neatCode);
                publishToRedis(dto);
            } else {
