...
/**
 * flight engine start change message processor
 *
...
@Service("FLOP_EGST")
@Slf4j
public class EgstProcessor extends AbsProcessor {
    private final FlightDAO flightDAO;

    @Autowired
    public EgstProcessor(FlightDAO flightDAO) {
        this.flightDAO = flightDAO;
    }

    /**
     * Process the EGST message.
     *
        ...
        if (null != flops && !flops.isEmpty()) {
            for (FLOP.FlopData record : flops) {
                if (null == record.getFLID()) {
                    log.error("There is no FFID tag in EGST message. Message text [" + msg + "]");
                    return;
                }
                ...
                // check flight engine start time tag
                String engineStartTimeStr = record.getEGST();
                if (null == engineStartTimeStr || engineStartTimeStr.isEmpty()) {
                    log.info("The existing engine start time value is being deleted. Message text [" + msg + "]");
                    if (null == flightDTO.getFlight().getEngineStart()) {
                        return;
                    }
                    updateEgst(flightDTO, null);
                    return;
                }
                try {
                    Date engineStartTime = ThreadLocalDateUtil.parseXmlDate(engineStartTimeStr);
                    if (null == engineStartTime) {
                        log.error("The format of EGST is wrong, Message text [" + msg + "], EGST text [" + engineStartTimeStr + "]");
                        return;
                    }
                    // if same, no update is made.
                    Date oldEngineStartTime = flightDTO.getFlight().getEngineStart();
                    if (null != oldEngineStartTime && engineStartTime.compareTo(oldEngineStartTime) == 0) {
                        log.info("EGST is same so nothing changed. Engine start time = [{}]", engineStartTime);
                    } else {
                        // do actual update
                        updateEgst(flightDTO, engineStartTime);
                    }
            ...
            log.error("There is no FLOP section in EGST message. Message text [" + msg + "]");
        }
    }
     ...
     * @param engineStartTime engine start time.
     */
    private void updateEgst(FlightDTO flightDTO, Date engineStartTime) {
        FlightDTO newFlightDTO = (FlightDTO) ObjectUtil.deepClone(flightDTO);
        if (null != newFlightDTO && null != newFlightDTO.getFlight()) {
            newFlightDTO.getFlight().setEngineStart(engineStartTime);
            ...
            FlightNotificationDTO dto = FlightNotificationDTO.buildUpdateNotification(flightDTO.getFlight().getId())
                    .addUpdateFields("engineStartTime",engineStartTime);
            publishToRedis(dto);
        } else {
