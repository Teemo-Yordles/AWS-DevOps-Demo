...
/**
 * flight naat type change message processor
 *
...
@Service("FLOP_NAAT")
@Slf4j
public class NaatProcessor extends AbsProcessor implements Processor {
    private final FlightDAO flightDAO;

    @Autowired
    public NaatProcessor(FlightDAO flightDAO) {
        this.flightDAO = flightDAO;
    }

    /**
     * Process the NAAT message.
     *
    ...
    public void process(String msg) throws JsonProcessingException {
        ...
        if (null == flops || flops.isEmpty()) {
            log.error("There is no FLOP section in NAAT message. Message text [" + msg + "]");
            return;
        }
        ...
        for (FLOP.FlopData record : flops) {
            if (null == record.getFLID()) {
                log.error("There is no FFID tag in NAAT message. Message text [" + msg + "]");
                return;
            }
            ...
            // check NAAT tag
            String naat = record.getNAAT();
            Date naatCode = null;
            if(naat == null){
                if (flightDTO.getFlight().getNaat() == null){
                    log.info("NAAT is same so nothing changed. NaatCode=null");
                    return;
                }
            }else{
                try {
                    naatCode = ThreadLocalDateUtil.parseXmlDate(naat);
                } catch (ParseException e) {
                    log.error("Parser NAAT xml date error.Message text [" + msg + "]" );
                    return;
                }
                // if same, no update is made.
                if (naatCode.equals(flightDTO.getFlight().getNaat())) {
                    log.info("NAAT is same so nothing changed. NaatCode=[{}]", naatCode);
                    return;
                }
            }
            // do actual update
            ...
            if (null != newFlightDTO && null != newFlightDTO.getFlight()) {
                newFlightDTO.getFlight().setNaat(naatCode);
                ...
                FlightNotificationDTO dto = FlightNotificationDTO.buildUpdateNotification(record.getFLID())
                        .addUpdateFields("naatCode",naatCode);
                publishToRedis(dto);
            } else {
